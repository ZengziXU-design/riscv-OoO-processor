...
from proj3.tinyrv2_encoding import TinyRV2Inst, disassemble_inst
...
class ProcFL( Component ):
  ...
  def construct( s, num_cores=1 ):
    ...
    s.imem_adapter = MemRequesterAdapterFL( MemReqMsg, MemRespMsg )
    s.dmem_adapter = MemRequesterAdapterFL( MemReqMsg, MemRespMsg )

    connect( s.imem, s.imem_adapter.requester )
    connect( s.dmem, s.dmem_adapter.requester )

    s.xcel_adapter = XcelRequesterAdapterFL( XcelReqMsg, XcelRespMsg )
