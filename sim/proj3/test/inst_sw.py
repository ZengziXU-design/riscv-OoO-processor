...
#-------------------------------------------------------------------------
# gen_simple_sw_lw_test
#-------------------------------------------------------------------------
# This test requires the processor to bypass the store data at the same
# time as making sure the immediate is properly sent into the op2
# register.

def gen_simple_sw_lw_test():
  return """
    csrr x1, mngr2proc < 0x00002000
    csrr x2, mngr2proc < 0xdeadbeef
    sw   x2, 0(x1)
